...
- Added:
    - `yazi.sh` to check for yazi and install if needed
...
- Moved COPR to `lionhardp/Hyprland`
    - `solopasha` hasn't been getting updates
- Added Hyprsunset to required packages
- AGSv1 kept as backup if Quickshell fails
- Hyprland-Dots now has switcher to fallback to AGS if QS fails or not isntalled
- Remove distro-specific notifier. Thanks Deekshith
